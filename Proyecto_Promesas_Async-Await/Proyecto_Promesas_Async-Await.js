  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (mesasSolicitadas <= mesasDisponibles) {
        ...
        console.log(`Ahora quedan ${mesasDisponibles} mesas disponibles`);
        resolve(`Reserva realizada con éxito`);
      } else {
        reject(`Lo sentimos, pero no hay mesas disponibles`);
      }
...
function enviarConfirmacionReserva(nombreCliente) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      let randomEmail = Math.floor(Math.random() * 11);
      if (randomEmail < 6) {
        resolve(`E-mail enviado con éxito a ${nombreCliente}`);
      } else {
        reject(`No se pudo enviar el e-mail`);
      }
...
async function hacerReserva(nombreCliente, mesasSolicitadas) {
  try {
    ...
    const email = await enviarConfirmacionReserva(nombreCliente);
    console.log(email);
